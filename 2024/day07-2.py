...
def evaluate(value: int, input: list[int], expected: int) -> bool:
    ...
    if len(input) == 1:
        return (
            value * input[0] == expected or value + input[0] == expected or concat_numbers(value, input[0]) == expected
        )

    return (
        evaluate(value * input[0], input[1:], expected)
        or evaluate(value + input[0], input[1:], expected)
        or evaluate(concat_numbers(value, input[0]), input[1:], expected)
    )
